...
public class OrderController {
    ...
    @RequestMapping(value = "/getSalesPriceDaily.do", method = RequestMethod.GET)
    @ApiOperation("获取当日销售额")
    public Float getSalesPriceDaily() {
        return orderService.getSalesPriceDaily();
    }

    @RequestMapping(value = "/getSalesDaily.do", method = RequestMethod.GET)
    @ApiOperation("获取当日总销售量")
    public Integer getSalesDaily() {
        return orderService.getSalesDaily();
    }

    @RequestMapping(value = "listTopSalesProductsDaily.do", method = RequestMethod.POST, produces = "application/json")
    @ApiOperation("统计今日销量TOP n")
    public List<ProductDaily> listTopSalesProducts(
            @RequestParam(value = "n", required = false) Integer n) {
        return orderService.listTopSalesProductsDaily(n);
    }

    @RequestMapping(value = "listTopSalesProductSimples.do", method = RequestMethod.POST, produces = "application/json")
    @ApiOperation("统计历史销量最高n位的ProductSimple")
    public List<ProductSimple> listTopSalesProductSimples(
            @RequestParam(value = "n", required = false) Integer n) {
        return orderService.listTopSalesProductSimples(n);
    }
    ...
    @RequestMapping(value = "createOrderByEmp.do", method = RequestMethod.POST, produces = "application/json")
    @ApiOperation("传入订单json单元创建订单")
    public Result getOrderByOrderId(@RequestBody OrderSubmit orderSubmit) {
        try {
            return orderService.createOrderByEmp(orderSubmit);
        } catch (Exception e) {
            e.printStackTrace();
            return new Result(false, "创建订单失败");
        }
